...
def gurobi_milp(prob:Instance, grb=None, opt_model=None):

    SJ = range(1, prob.numJob)
    SM = range(1, prob.numMch)
    s = prob.setup
    p = prob.ptime
    M = 0
    ...
    #제약조건
    constraint_1 = {(i, k): opt_model.addConstr(lhs=grb.quicksum(C_ik[i, k] + S_ik[i, k] for i in SJ for k in SM),sense=grb.GRB.LESS_EQUAL,rhs=M * y_ik[i, k],
            name="constraint1_{0}_{1}".format(i, k)) for i in SJ for k in SM}
    constraint_2 = {(i, k): opt_model.addConstr(lhs=grb.quicksum(C_ik[i, k] for i in SJ for k in SM),sense=grb.GRB.GREATER_EQUAL, rhs=S_ik[i, k] + p[k][i] - M * (1 - y_ik[i, k]),
            name="constraint2_{0}_{1}".format(i, k)) for i in SJ for k in SM}
    constraint_3 = {(i, j, k): opt_model.addConstr(lhs=grb.quicksum(S_ik[i, k] for i in SJ for k in SM), sense=grb.GRB.GREATER_EQUAL, rhs=C_ik[j, k] + s[k][j][i]*y_ik[j, k] - M2*z_ijk[i, j, k],
                                                name="constraint3_{0}_{1}_{2}".format(i, j, k)) for k in SM for i in SJ for j in SJ if i < j}
    constraint_4 = {(i, j, k): opt_model.addConstr(lhs=grb.quicksum(S_ik[j, k] for j in SJ for k in SM), sense=grb.GRB.GREATER_EQUAL,
